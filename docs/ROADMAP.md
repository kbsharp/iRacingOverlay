...
- **This app** sells judgement: iRacing-only, opinionated, zero-config, *right by
  default*. A strong core — five widgets finished properly plus the strategy
  layer nobody else has — not a wide catalogue. **A race engineer, not a
  dashboard.**
...
- **Full column-level customization.** Both competitors sell it hard; it's their
  moat and our anti-feature. Every column here earns its place and **the default
  *is* the product**. Modest show/hide toggles are fine; a layout editor is not.
...
| Traffic meeting-point forecast | ❌ | behind-only warning | ✅ **(unique)** — faster class + sector |
