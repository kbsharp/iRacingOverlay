...
/// within its class, with best/last lap times, a last-lap delta, the interval
/// to the car ahead, the gap to the class leader, and the class Strength of
/// Field.
///
/// Gaps come from iRacing's CarIdxF2Time (time behind the session leader);
/// subtracting the class leader's value (for the gap) or the car-ahead's value
/// (for the interval) gives a within-class figure that is correct regardless of
/// whether F2Time is measured against the overall or class leader, since it's a
...
public static class StandingsCalculator
{
        ...
        string className, List<Entry> classEntries, int playerCarIdx, int maxPerClass, float sessionBest)
    {
        ...
        var leaderBest = leader.BestLapTimeSeconds;
        var classColor = classEntries
            .Select(e => e.Driver?.ClassColorRaw)
            .FirstOrDefault(c => c is not null);
        var sof = StrengthOfField.Compute(classEntries.Select(e => e.Driver?.IRating ?? 0));
        ...
        var group = new StandingsClassGroup(className, RatingFormat.NormalizeHexColor(classColor), sof, shown);
        ...
        float? prevF2, string className, int playerCarIdx, float sessionBest)
    {
        ...
        var gap = isLeader ? 0 : PositiveOrNull(car.F2TimeSeconds - leaderF2);
        var interval = prevF2 is null ? null : PositiveOrNull(car.F2TimeSeconds - prevF2.Value);

        var gapLapsDown = GapLapsDownFor(gap, isLeader, leaderBestLap, leader, car);
        var intervalLapsDown = interval is not null && leaderBestLap > 0 ? (int)(interval.Value / leaderBestLap) : 0;
        ...
        var last = car.LastLapTimeSeconds > 0 ? car.LastLapTimeSeconds : (double?)null;
        ...
        return new StandingsRow(
            ...
            GapLapsDown: gapLapsDown,
            IntervalSeconds: interval,
            IntervalLapsDown: intervalLapsDown,
            IsSessionBestLap: isSessionBest,
            InPits: car.OnPitRoad || car.Surface == CarTrackSurface.InPitStall);
    }

    // Laps down comes from the time gap versus the class leader's lap time, not
    // a raw completed-lap difference - the latter flickers to "+1L" for a car
    // only tenths behind whenever the leader has just crossed the line. Fall
    // back to the lap-count difference only when no lap time is known.
    private static int GapLapsDownFor(
        double? gap, bool isLeader, float leaderBestLap, in CarTelemetry leader, in CarTelemetry car)
    {
        ...
        if (leaderBestLap > 0)
        {
            return (int)(gap.Value / leaderBestLap);
        }

        return leader.LapsCompleted >= 0 && car.LapsCompleted >= 0
            ? Math.Max(0, leader.LapsCompleted - car.LapsCompleted)
            : 0;
    }

    private static double? PositiveOrNull(float value) => value > 0 ? value : null;
